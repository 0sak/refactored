...
public class TestElifStatement {
    ...
    private static Stream<Arguments> sources() {
        return Stream.of(
                Arguments.of("if(False):print('Went into if Statement')elif(True):print('Went into elif Statement')#end#end",
                        "Went into elif Statement\n"),
                Arguments.of("if(True):print('Went into if Statement')elif(False):print('Went into elif Statement')#end#end",
                        "Went into if Statement\n"));
    }
    @ParameterizedTest
    @MethodSource("sources")
    void elif_statement(String input, String expected, @TempDir Path workDirectory) throws IOException, InterruptedException {
        String result = "";
